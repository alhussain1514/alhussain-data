  ...
  const saveEdit = async (plan) => {
    ...
    try {
      const updatedPlans = plans.map((p) =>
        p.id === plan.id ? { ...p, sellingPrice: newPrice, price: newPrice } : p
      )
    ...
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">Update selling prices for all data plans</p>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {NETWORKS.map((net) => (
          <button key={net} onClick={() => setActiveNetwork(net)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeNetwork === net ? 'bg-brand-blue/20 text-brand-blue border border-brand-blue/30' : 'glass-card text-slate-400 hover:text-white'}`}>
            {net}
          </button>
        ))}
      </div>
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-400 text-sm">No active plans for {activeNetwork}.</div>
        ) : (
              ...
              {networkPlans.map((plan) => (
                <tr key={plan.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02]">
                    ...
                    {editingId === plan.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <input type="number" value={editValue}
                          ...
                          className="w-24 bg-white/10 border border-white/20 rounded-lg px-2 py-1 text-white text-sm focus:outline-none focus:border-brand-blue" />
                      </div>
                    ) : (
                  ...
                  <td className="px-5 py-3">
                    {editingId === plan.id ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => saveEdit(plan)} disabled={saving}
                          className="w-7 h-7 rounded-lg bg-emerald-400/20 flex items-center justify-center text-emerald-400 hover:bg-emerald-400/30">
                          <Check size={13} />
                        </button>
                      ...
                      <button onClick={() => { setEditingId(plan.id); setEditValue(String(plan.sellingPrice || plan.price)) }}
                        className="w-7 h-7 rounded-lg bg-white/5 hover:bg-brand-blue/20 flex items-center justify-center text-slate-400 hover:text-brand-blue transition-colors">
                        <Edit2 size={13} />
                      </button>
                    )}
                  </td>
      ...
      <div className="glass-card p-5">
        ...
        <p className="text-xs text-slate-400 mb-4">These prices are set in the backend code. To update them, edit PIN_PRICES in src/controllers/vtu.controller.js in Codespaces.</p>
        <div className="grid grid-cols-3 gap-3">
          {[{label:'WAEC',price:3600},{label:'NECO',price:1500},{label:'NABTEB',price:1200}].map((e) => (
            <div key={e.label} className="bg-white/5 rounded-xl p-4 text-center">
              <p className="text-sm font-semibold text-white">{e.label}</p>
              <p className="font-display font-bold text-emerald-400 mt-1">{formatNaira(e.price)}</p>
            </div>
          ))}
        </div>
      </div>
