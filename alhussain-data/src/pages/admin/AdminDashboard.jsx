  ...
  return (
    <div className="space-y-6 max-w-6xl">
        ...
        {cards.map((c) => {
          ...
          return (
            <div key={c.label} className="glass-card p-5">
              ...
              {loading ? <div className="h-7 w-24 bg-white/5 rounded animate-pulse mb-1" /> : <p className="font-display text-2xl font-bold text-white">{c.value}</p>}
              <p className="text-xs text-slate-400 mt-1">{c.label}</p>
              <p className="text-xs text-emerald-400 mt-1.5 flex items-center gap-1"><ArrowUpRight size={11} />{c.sub}</p>
            </div>
          )
        })}
      </div>
      <div className="glass-card p-5">
        ...
        {loading ? (
          <div className="space-y-3">{[...Array(5)].map((_, i) => (<div key={i} className="flex items-center gap-3"><div className="w-7 h-7 rounded-full bg-white/5 animate-pulse" /><div className="flex-1 h-3 bg-white/5 rounded animate-pulse" /></div>))}</div>
        ) : recent.length === 0 ? (
                ...
                {recent.map((tx) => {
                  ...
                  return (
                    <tr key={tx._id} className="border-b border-white/[0.04] last:border-0">
                      <td className="py-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-gradient-brand flex items-center justify-center text-xs font-bold text-white flex-shrink-0">{tx.user?.name?.[0]?.toUpperCase() || '?'}</div><span className="text-white font-medium">{tx.user?.name || 'Unknown'}</span></div></td>
