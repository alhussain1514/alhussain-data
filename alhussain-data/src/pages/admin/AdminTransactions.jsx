...
export default function AdminTransactions() {
  ...
  const TYPES = ['data', 'airtime', 'electricity', 'tv', 'wallet_fund', 'result_checker']
    ...
    <div className="space-y-5 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">{total.toLocaleString()} total transactions</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" placeholder="Search user, phone, reference..." value={search} onChange={(e) => setSearch(e.target.value)} className="input-field pl-10 w-full" />
        </div>
        ...
        {(statusFilter || typeFilter) && (
          <button onClick={() => { setStatusFilter(''); setTypeFilter(''); setPage(1) }} className="btn-ghost text-xs gap-1"><X size={12} /> Clear</button>
        )}
      </div>
                ...
                {filtered.map((tx) => {
                  ...
                  return (
                    <tr key={tx._id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02]">
                      <td className="px-5 py-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-brand-blue/15 flex items-center justify-center text-xs font-bold text-brand-blue flex-shrink-0">{tx.user?.name?.[0]?.toUpperCase() || '?'}</div><div><p className="text-white font-medium text-xs">{tx.user?.name || 'Unknown'}</p><p className="text-xs text-slate-500">{tx.user?.phone}</p></div></div></td>
                      <td className="px-5 py-3 text-slate-300 text-xs">{typeInfo.icon} {typeInfo.label}</td>
                      <td className="px-5 py-3 text-slate-400 text-xs max-w-32 truncate">{tx.description || '-'}</td>
                      ...
                      <td className="px-5 py-3"><button onClick={() => setSelected(tx)} className="text-xs text-brand-blue hover:text-brand-cyan transition-colors">View</button></td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
          <div className="glass-card p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-display font-semibold">Transaction Receipt</h3>
            ...
            <div className="text-center mb-5 pb-4 border-b border-white/10">
              <p className="font-display font-bold">AL-HUSSAIN <span className="text-brand-cyan">DATA</span></p>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={k} className="flex justify-between text-sm">
                  ...
                  <span className="text-white font-medium text-right max-w-48 truncate">{v}</span>
                </div>
              ))}
            </div>
