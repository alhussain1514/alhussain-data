...
export default function AdminUsers() {
  ...
  const handleFund = async () => {
    ...
    try {
      ...
      if (selectedUser?._id === fundModal._id) setSelectedUser((prev) => ({ ...prev, walletBalance: res.data.balance }))
  ...
  const handleStatus = async (user, status) => {
    try {
      ...
      if (selectedUser?._id === user._id) setSelectedUser((prev) => ({ ...prev, status }))
                  ...
                  <tr key={u._id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02]">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="text-white font-medium">{u.name}</p><p className="text-xs text-slate-500">{u.email}</p></div>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-slate-300">{u.phone}</td>
                    <td className="px-5 py-3 font-medium text-emerald-400">{formatNaira(u.walletBalance || 0)}</td>
                    <td className="px-5 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${u.status === 'active' ? 'bg-emerald-400/10 text-emerald-400' : 'bg-red-400/10 text-red-400'}`}>{u.status || 'active'}</span>
                    </td>
                    <td className="px-5 py-3 text-slate-500 text-xs">{formatDateShort(u.createdAt)}</td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => viewUser(u)} title="View" className="w-7 h-7 rounded-lg bg-white/5 hover:bg-brand-blue/20 flex items-center justify-center text-slate-400 hover:text-brand-blue transition-colors"><Eye size={13} /></button>
      ...
      {fundModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setFundModal(null)}>
          <div className="glass-card p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-400 mb-4">Crediting <span className="text-white font-medium">{fundModal.name}</span></p>
            ...
            <div className="mb-4">
              ...
              <input type="number" autoFocus placeholder="Enter amount" value={fundAmount} onChange={(e) => setFundAmount(e.target.value)} className="input-field" />
            </div>
            <div className="flex gap-2">
              <button onClick={() => setFundModal(null)} className="btn-ghost flex-1 justify-center">Cancel</button>
              <button onClick={handleFund} disabled={funding || !fundAmount} className="btn-primary flex-1 justify-center disabled:opacity-60">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedUser(null)}>
          <div className="glass-card p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-display font-semibold text-lg">User Details</h3>
            ...
            <div className="flex items-center gap-4 mb-5 p-4 bg-white/5 rounded-xl">
              <div className="w-12 h-12 rounded-full bg-brand-blue/20 flex items-center justify-center text-lg font-bold text-brand-blue">{selectedUser.name?.[0]?.toUpperCase()}</div>
              <div>
              ...
              <div className="ml-auto text-right">
                ...
                <p className="font-display font-bold text-emerald-400">{formatNaira(selectedUser.walletBalance || 0)}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-5">
              ...
              {selectedUser.status !== 'suspended' ? (
                <button onClick={() => handleStatus(selectedUser, 'suspended')} className="btn-ghost justify-center gap-2 text-sm text-red-400 hover:text-red-400"><UserX size={14} /> Suspend</button>
              ) : (
                <button onClick={() => handleStatus(selectedUser, 'active')} className="btn-ghost justify-center gap-2 text-sm text-emerald-400 hover:text-emerald-400"><UserCheck size={14} /> Activate</button>
              )}
            </div>
            ...
            {loadingUser ? (
              ...
              <div className="text-center py-4 text-slate-400 text-sm">No transactions yet.</div>
            ) : (
              <div className="space-y-2">
                {userTx.map((tx) => (
                  <div key={tx._id} className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
                    <div>
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-white">{formatNaira(tx.amount)}</p>
