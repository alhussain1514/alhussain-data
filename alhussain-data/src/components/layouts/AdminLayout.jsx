...
import { LayoutDashboard, Users, Receipt, Tag, LogOut, Menu, Shield } from 'lucide-react'
      ...
      <aside className="hidden md:flex w-60 h-full flex-col bg-navy-2 border-r border-white/[0.06]">
          ...
          <ul className="space-y-0.5 list-none">
            {ADMIN_NAV.map((item) => {
              ...
              return (
                <li key={item.path}>
                  <Link to={item.path}
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${active ? 'bg-brand-purple/15 text-brand-purple border border-brand-purple/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                    <Icon size={17} />
                    {item.label}
                  </Link>
                </li>
        ...
        <div className="px-3 pb-4 border-t border-white/[0.06] pt-4">
          ...
          <button onClick={handleLogout}
            className="flex items-center gap-3 px-3 py-2.5 w-full rounded-xl text-sm font-medium text-slate-400 hover:text-red-400 hover:bg-red-400/5 transition-all">
